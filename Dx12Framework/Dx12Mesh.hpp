...
namespace Dx12Framework {
	...
	public:
		Dx12Mesh(Dx12Framework::Dx12DeviceContext& dc, std::span<T_Vertex> vertices, std::span<uint32_t> indices) {
			...
			// Write triangle data into staging buffer
			{
				...
				pMappedData = static_cast<uint8_t*>(pMappedData) + vertices.size_bytes();
				memcpy(pMappedData, indices.data(), indices.size_bytes());
			...
			// Copy to default buffer
			{
				...
				pDx12CopyList->CopyBufferRegion(pDxVertexBuffer.Get(), 0, pDxStagingBuffer.Get(), 0U, vertices.size_bytes());
				pDx12CopyList->Close();
