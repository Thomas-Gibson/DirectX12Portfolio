...
// For Dx12ApplicationLauncher to initialize & run Dx12TriangleApp
extern "C" _declspec(dllexport)
Dx12Framework::Dx12Application* GetApplication(const Dx12Framework::Dx12DeviceContext& dc, ImGuiContext* imgui) {
	return new Dx12TriangleApp(dc, imgui);
}

Dx12TriangleApp::Dx12TriangleApp(const Dx12Framework::Dx12DeviceContext& dc, ImGuiContext* imgui) : Dx12Application(dc, imgui)
{
	InitPipeline();
	LoadTriangle();
}
...
void Dx12TriangleApp::InitPipeline() {
	...
	ThrowIfFailed(D3DReadFileToBlob(L"Dx12Triangle/VertexShader.cso", VertexShader.GetAddressOf()));
	ThrowIfFailed(D3DReadFileToBlob(L"Dx12Triangle/PixelShader.cso", Pixelshader.GetAddressOf()));
